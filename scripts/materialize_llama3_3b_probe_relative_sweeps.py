...
BASE_CONFIG = ROOT / "configs/llama3_3b_mask_compare_probe_relative_main_sweep.yaml"
...
def main() -> None:
    ...
    for value in VALUES:
        ...
        cfg["output_dir"] = (
            f"outputs/llama3_3b_mask_compare_probe_relative_alpha_sweep/alpha/{stem}"
        )
        write(
            ROOT / "configs/llama3_3b_mask_compare_probe_relative_alpha_sweep/alpha"
            / f"{stem}.yaml",
            cfg,
        )
    ...
    for value in VALUES:
        ...
        cfg["unlearn"]["shared_methods"] = []
        cfg["output_dir"] = (
            f"outputs/llama3_3b_mask_compare_probe_relative_beta_sweep/beta/{stem}"
        )
        write(
            ROOT / "configs/llama3_3b_mask_compare_probe_relative_beta_sweep/beta"
            / f"{stem}.yaml",
            cfg,
        )
    ...
    sweep_config_root = (
        ROOT
        / "configs/llama3_3b_mask_compare_probe_relative_main_sweep"
    )
    sweep_output_root = (
        "outputs/llama3_3b_mask_compare_probe_relative_main_sweep"
    )
