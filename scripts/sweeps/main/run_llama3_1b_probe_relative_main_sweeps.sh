...
shared_methods = {"grad_ascent", "grad_diff", "npo", "no_mask"}
shared_root = None
...
fd, temp_path = tempfile.mkstemp(prefix="llama3_1b_main_sweep_", suffix=".yaml")
os.close(fd)
...
case "$MODE" in
  activity)
    run_pattern "configs/llama3_1b_mask_compare_probe_relative_main_sweep/activity/*.yaml"
    ;;
  margin)
    run_pattern "configs/llama3_1b_mask_compare_probe_relative_main_sweep/margin/*.yaml"
    ;;
  all)
    run_pattern "configs/llama3_1b_mask_compare_probe_relative_main_sweep/activity/*.yaml"
    run_pattern "configs/llama3_1b_mask_compare_probe_relative_main_sweep/margin/*.yaml"
