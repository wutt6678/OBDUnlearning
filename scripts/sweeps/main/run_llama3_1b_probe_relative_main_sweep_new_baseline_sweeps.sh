...
fd, temp_path = tempfile.mkstemp(prefix="llama3_1b_new_baseline_sweep_", suffix=".yaml")
os.close(fd)
...
run_objective() {
  ...
  case "$mode" in
    activity)
      run_pattern "configs/llama3_1b_mask_compare_probe_relative_main_sweep_${objective}_sweep/activity/*.yaml"
      ;;
    margin)
      run_pattern "configs/llama3_1b_mask_compare_probe_relative_main_sweep_${objective}_sweep/margin/*.yaml"
      ;;
    all)
      run_pattern "configs/llama3_1b_mask_compare_probe_relative_main_sweep_${objective}_sweep/activity/*.yaml"
      run_pattern "configs/llama3_1b_mask_compare_probe_relative_main_sweep_${objective}_sweep/margin/*.yaml"
      ;;
    *)
      echo "Usage: $0 [all|dpo|simnpo|rmu|kl|retrain] [activity|margin|all]" >&2
      exit 1
      ;;
  esac
}

"$PYTHON_BIN" "$ROOT_DIR/scripts/sweeps/main/materialize_llama3_1b_probe_relative_main_sweep_new_baseline_sweeps.py"
