...
obj["unlearn"]["baselines"] = [
    method for method in sys.argv[2].split(",") if method
]
fd, temp_path = tempfile.mkstemp(prefix="llama3_1b_beta_objective_sweep_", suffix=".yaml")
os.close(fd)
...
run_objective() {
  local objective="$1"
  run_pattern "configs/llama3_1b_mask_compare_probe_relative_beta_objective_sweeps/${objective}_beta_sweep/beta/*.yaml"
}
...
if [ "$OBJECTIVE" = "all" ]; then
  ...
  case "$OBJECTIVE" in
    grad_diff|grad_ascent|npo|dpo|simnpo|rmu|kl|retrain)
      ...
      ;;
    *)
      echo "Usage: $0 [all|grad_diff|grad_ascent|npo|dpo|simnpo|rmu|kl|retrain]" >&2
      exit 1
      ;;
  esac
fi
