...
fd, path = tempfile.mkstemp(prefix="ministral3_3b_instruct_sweep_", suffix=".yaml")
os.close(fd)
...
case "$MODE" in
  alpha)
    run_pattern "configs/ministral3_3b_instruct_mask_compare_probe_relative_alpha_sweep/alpha/*.yaml"
    ;;
  beta)
    run_pattern "configs/ministral3_3b_instruct_mask_compare_probe_relative_beta_sweep/beta/*.yaml"
    ;;
  activity)
    run_pattern "configs/ministral3_3b_instruct_mask_compare_probe_relative_main_sweep/activity/*.yaml"
    ;;
  margin)
    run_pattern "configs/ministral3_3b_instruct_mask_compare_probe_relative_main_sweep/margin/*.yaml"
    ;;
  stronger|main)
    run_pattern "configs/ministral3_3b_instruct_mask_compare_probe_relative_main_sweep/activity/*.yaml"
    run_pattern "configs/ministral3_3b_instruct_mask_compare_probe_relative_main_sweep/margin/*.yaml"
  ...
  all)
    ...
    ;;
  *)
    echo "Usage: $0 [alpha|beta|activity|margin|main|all]" >&2
    exit 1
    ;;
esac
