...
if "baselines" in unlearn_cfg:
    ...
    objective = unlearn_cfg.get("objective", "grad_ascent")
    wrappers = unlearn_cfg.get(
...
for method in methods:
    ...
    if (
        ...
        and method in shared_methods
        and (shared_root / method / "open_tofu_eval.json").exists()
    ):
        completed.add(method)
...
fd, temp_path = tempfile.mkstemp(
    prefix="llama3_1b_grad_ascent_sweep_",
    suffix=".yaml",
)
os.close(fd)
...
case "$MODE" in
  activity)
    run_pattern "configs/llama3_1b_mask_compare_probe_relative_main_sweep_grad_ascent_sweep/activity/*.yaml"
    ;;
  margin)
    run_pattern "configs/llama3_1b_mask_compare_probe_relative_main_sweep_grad_ascent_sweep/margin/*.yaml"
    ;;
  all)
    run_pattern "configs/llama3_1b_mask_compare_probe_relative_main_sweep_grad_ascent_sweep/activity/*.yaml"
    run_pattern "configs/llama3_1b_mask_compare_probe_relative_main_sweep_grad_ascent_sweep/margin/*.yaml"
