...
SWEEP_SUBDIR = "llm_model_unlearning_method_default_sweep"
# Aggregate artifacts live in a reserved dir that never holds a model sweep.
PLOT_DIR = SWEEP_ROOT / SWEEP_SUBDIR / "plots"
...
def collect_rows() -> list[dict]:
    ...
    for model_dir in sorted(SWEEP_ROOT.iterdir()):
        ...
        if not model_dir.is_dir() or not sweep_dir.is_dir():
            continue
        model = model_dir.name
        for objective in OBJECTIVES:
            comparison_path = sweep_dir / objective / "open_tofu_comparison.json"
...
def make_markdown(rows: list[dict]) -> None:
    ...
    out_path = SWEEP_ROOT / SWEEP_SUBDIR / "summary.md"
    lines = [
    ...
    out_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    (SWEEP_ROOT / SWEEP_SUBDIR / "summary.json").write_text(json.dumps(rows, indent=2), encoding="utf-8")
    print(f"saved {out_path}")
    print(f"saved {SWEEP_ROOT / SWEEP_SUBDIR / 'summary.json'}")
