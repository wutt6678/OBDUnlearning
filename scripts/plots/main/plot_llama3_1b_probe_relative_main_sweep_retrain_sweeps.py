...
OBJECTIVE = "retrain"
SWEEP_ROOT = ROOT / "outputs/sweeps/llama3_1b/main/mask_compare_probe_relative_main_sweep_retrain_sweep"
...
def load_runs(group: str, pattern: str) -> list[tuple[float, dict]]:
    paths = sorted((SWEEP_ROOT / group).glob(f"{pattern}/open_tofu_comparison.json"))
    runs = []
    for path in paths:
...
def main() -> None:
    ...
    for group, pattern, x_label in FIGURES:
        runs = load_runs(group, pattern)
        if not runs:
            print(f"skipping {group}: no completed runs under {SWEEP_ROOT / group}")
            continue
        ...
        fig.suptitle(f"Llama3-1B probe-relative main_sweep {OBJECTIVE}: {group}", y=1.02)
        fig.tight_layout(rect=(0, 0.06, 1, 0.96))
        out_path = PLOT_DIR / f"{group}_curve_forget_rouge_l_utility_score_mia_auc.png"
