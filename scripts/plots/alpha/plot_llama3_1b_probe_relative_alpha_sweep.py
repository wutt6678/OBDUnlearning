...
SWEEP_ROOT = ROOT / "outputs/sweeps/llama3_1b/alpha/mask_compare_probe_relative_alpha_objective_sweeps/grad_diff_alpha_sweep"
...
FIGURES = [
    (
        "alpha",
        sorted((SWEEP_ROOT / "alpha").glob("alpha_*/open_tofu_comparison.json")),
        "alpha_forget",
    ),
]
...
def metric_series(run: dict, method_key: str, metric_key: str) -> float:
...
def plot_curve(fig_kind: str, runs: list[dict], x_label: str, baseline: dict[str, float]) -> None:
    ...
    for ax, (metric_key, metric_title) in zip(axes, METRICS):
        ...
        for method_key, method_label in METHODS:
            ys = [metric_series(item["run"], method_key, metric_key) for item in runs]
            ax.plot(
                xs,
                ys,
                marker="o",
    ...
    fig.suptitle(f"Llama3-1B probe-relative alpha_forget sweep: {fig_kind}", y=1.02)
