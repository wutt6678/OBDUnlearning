...
fd, temp_path = tempfile.mkstemp(prefix="llama3_1b_dynamic_k5_alpha_sweep_", suffix=".yaml")
os.close(fd)
...
case "$MODE" in
  alpha|all)
    run_pattern "configs/llama3_1b_mask_compare_probe_relative_dynamic_k5_alpha_sweep/alpha/*.yaml"
    ;;
  *)
    echo "Usage: $0 [alpha|all]" >&2
    exit 1
    ;;
esac
